...
\subsection{CD with 1-step Reconstruction}
	...
	The second term is perfect fit for Gibbs sampling, since we can $ p(\mathbf{v}, \mathbf{h} \mid \Theta) $ as a 2D vector and generating the samples by jumping in a Markov Chain with the transformation probability matrix $ p(\mathbf{v} \mid \mathbf{h}, \Theta) $ and $ p(\mathbf{h} \mid \mathbf{v}, \Theta) $, see Figure~ and Section~\ref{sec:Gibbs}.
...
%			\EndFor
	...
	\begin{figure}[hbt]
		\centering
		\includegraphics[width=0.6\textwidth]{img/gibbs.png}
		\caption{Gibbs sampling on RBM.}
		\label{fig:gibbs}
	\end{figure}
\subsection{Case Study on Deep Belief Network~\cite{hinton2006fast}}
