...
\section{Why CD(Contrastive Divergence)?\cite{hinton2002training,woodfordnotes}}
	...
	%  = K(<\log f(\mathbf{x} \mid \Theta )>_{data} - \log Z( \Theta))
	\end{equation}
...
\section{RBM\cite{zhang2013rbm}}
	\begin{itemize}
		...
		\begin{equation}
			f(\mathbf{X} \mid \Theta) =\exp (E(\mathbf{v}, \mathbf{h} \mid \Theta)) = \exp (-\sum_{i=1}^n a_i v_i - \sum_{j=1}^m b_j h_j - \sum_{i=1}^n \sum_{j=m}^n v_i W_{ij} h_j).
		\end{equation}
	\end{itemize}
