...
\subsection{CD Instead of KL(Kullback-Leibler)}
	...
	\end{aligned}
	\end{equation}
	and the second term of the right hand side is the average log-likelihood function see Equation~\ref{equ:like} if we see $P(\mathbf{x})$ is the training data distribution and $Q(\mathbf{x})$ is the model distribution:
	\begin{equation}
	\begin{aligned}
	...
	\end{aligned}
	\end{equation}
	and $\left \langle \log p(\mathbf{d} \mid \mathbf{D}) \right \rangle_{\mathbf{D}}$ is independent with model parameters.
	...
	Thus the negative partial derivative is the same with Equation~(\ref{equ:part}):
	\begin{equation}
	\label{equ:kl}
		...
		= \left \langle \dfrac{\partial \log f(\mathbf{d} \mid \Theta)}{\partial \theta}\right \rangle_{\mathbf{D}} - \left \langle \dfrac{\partial \log f(\mathbf{c} \mid \Theta)}{\partial \theta}\right \rangle_{\mathbf{C} \sim p(\mathbf{x} \mid \Theta)} .
	\end{equation}
	Therefore, for each iteration of search in the parameter space we measure $ k $ number of training data $ \mathbf{d} $ and fantasy data $\mathbf{c} $.
	If $ k $ is big enough, we believe it is an approximation of KL.
	However, if we just take a small number of data even $ k = 1 $ per iteration we call the small data derivative ``k-step contrastive convergence ($ CD_{k}) $'' and it will significantly reduce the training time.
%	Thus,
%	\begin{equation}
